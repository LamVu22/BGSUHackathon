...
export default function GraphPreview() {
  return (
    <div className="bg-base-100 border border-base-200 rounded-2xl p-6 h-full shadow-inner">
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="btn btn-xs btn-outline">Expand</button>
      </div>
      <div className="grid grid-cols-2 gap-4">
        {sampleNodes.map((node) => (
          <div
            key={node.id}
            className="p-3 rounded-xl border border-dashed border-primary/30 flex flex-col gap-1"
          >
